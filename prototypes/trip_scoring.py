...
def trip_creator(needed, inventory, storage):
    goal = ''
    batch = []
    trip = []
    for tier in tier6:
    ...
    goal = random.choice(batch)
    print()
    print(goal)
    # print(recipes[goal])
    trip.append(random.choice(recipes[goal]))
    ...
    for recipe in trip:
        ...
        # Item 1
        if recipe[0] not in shop_items and recipe[0] != '':
            goal = recipe[0]
            # print(goal)
            trip.append(random.choice(recipes[goal]))
            # print()
        ...
        # Item 2
        if recipe[1] not in shop_items and recipe[1] != '':
            goal = recipe[1]
            # print(goal)
            trip.append(random.choice(recipes[goal]))
            # print()
...
def trip_score():
    needed = ['Spicy Soup', 'Snow Cone', 'Electro Pop', 'Meteor Meal', 'Megaton Dinner', 'Block Meal', 'Courage Shell', 'Mistake', 'Golden Meal', 'Fried Shroom Plate', 'Roast Shroom Dish', 'Shroom Steak', 'Slimy Extract', 'Dangerous Delight', 'Sweet Cookie Snack', 'Fried Egg', 'Spaghetti Plate', 'Hamburger', 'Roast Meat', 'Peach Juice', 'Mango Juice', 'Sky Juice', 'Town Special', 'Warm Cocoa', 'Honey Candy', 'Fire Burst', 'Inky Soup', 'Dayzee Syrup', 'Sap Syrup', 'Healthy Salad', 'Koopa Tea', 'Herb Tea', 'Gold Bar', 'Roast Whacka Bump', 'Roast Horsetail', 'Emergency Ration', 'Space Food', 'Volt Shroom', 'Heavy Meal', 'Mushroom Crepe', 'Shroom Cake', 'Egg Bomb', 'Shroom Pudding', 'Love Pudding', 'Mousse', 'Hot Dog', 'Meat Pasta Dish', 'Gorgeous Steak', 'Fruity Shroom', 'Gradual Syrup', 'Peach Tart', 'Fruity Hamburger', 'Mango Pudding', 'Fruity Punch', 'Fruity Cake', 'Omelette Plate', 'Primordial Dinner', 'Lovely Chocolate', 'Shroom Choco-bar', 'Standard Chocolate', 'Chocolate Cake', 'Choco Pasta Dish', 'Golden Choco-bar', 'Sweet Choco-bar', 'Honey Shroom', 'Honey Super', 'Volcano Shroom', 'Awesome Snack', 'Spicy Dinner', 'Spicy Pasta Dish', 'Ink Pasta Dish', 'Stamina Juice', 'Shroom Delicacy', 'Sap Muffin', 'Weird Extract', 'Snow Bunny', 'Koopa Dumpling', 'Koopasta Dish', 'Koopa Pilaf', 'Shroom Broth', 'Horsetail Tart', 'Fruit Parfait', 'Life Shroom', 'Super Shroom Shake', 'Heartful Cake', 'Love Noodle Dish', 'Dyllis Breakfast', 'Dyllis Lunch', 'Odd Dinner', 'Dyllis Dinner', 'Veggie Set', 'Mixed Shake', 'Dyllis Special', 'Dyllis Deluxe', 'Gingerbread House', 'Berry Snow Bunny', 'Couple\'s Cake', 'Trial Stew', 'Luxurious Set', 'Dyllis Dynamite']
    trip = []               # Place recipes in here.
    storage = []            # Reference in order to see if the item is already obtained 
    start_inventory = []    # Use to see the start items that we need at the beginning of the trip
    item_tier = []          # 
    already_crafted = []    # 
    points = 0

    storage = ['Turtley Leaf', 'Turtley Leaf', 'Ice Storm', 'Ice Storm']
    start_inventory = ['Turtley Leaf', 'Turtley Leaf', 'Ice Storm', 'Ice Storm', 'Mighty Tonic', 'Mighty Tonic', 'Peachy Peach']

    trip = [
        ('Turtley Leaf', 'Ice Storm', 'Snow Bunny'),
        ('Turtley Leaf', 'Ice Storm', 'Snow Bunny'),
        ('Peachy Peach', 'Mighty Tonic', 'Peach Juice'),
        ('Peach Juice', 'Mighty Tonic', 'Mixed Shake'),
        ('Mixed Shake', 'Snow Bunny', 'Berry Snow Bunny'),
        ('Berry Snow Bunny', 'Snow Bunny', 'Couple\'s Cake')
    ]

    trip_length = len(trip) # 
    
    print()
    print("Apply points for using items.")
    for item in start_inventory:
        print(f'{difficulty[item]} {item}')
        points -= difficulty[item]      # subtract points for using difficult items
        if item in storage:
            storage.remove(item)
        else:
            points -= difficulty[item]  # if you have to obtain the item subtract again.
            
    print()
    print("Apply points for each recipe crafted")
    for recipe in trip:
        item1, item2, product = recipe
        print(tiers[product] * 10, product)
        if product in needed:
            points += tiers[product] * 10
            needed.remove(product)
        else:
            points -= tiers[product] * 10

    print()
    print("Apply multiplier for the amount of recipes crafted.")
    print(f"{points} * {trip_length} = {points * trip_length}")
    points *= trip_length
...
trip_creator(needed, inventory, storage)
